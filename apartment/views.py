...
def reservation(request, card_id):
    if request.POST:
        ...
        if form_data_booking.is_valid():
            ...
            message_booking = f'''
            Ваша заявка на бронирование квартиры принята. 
            Ссылка на квартиру: https://gostivl.ru/card/{card_id}/.
            Даты бронирования: с {date_start} до {date_end}.
            Статус: Ожидание подтверждения.

            Мы рады, что вы выбрали наш сервис для бронирования. Пока ваша заявка на рассмотрении, предлагаем вам ознакомиться с рядом полезных ресурсов для вашего пребывания в Владивостоке.

            1. **Рестораны**: Узнайте лучшие рестораны города
            2. **Развлечения**: Посмотрите, какие развлечения предлагает Владивосток
            3. **Мероприятия**: Ознакомьтесь с актуальными мероприятиями в городе
            

            С уважением, команда GOSTI VLADIVOSTOK.
            '''
